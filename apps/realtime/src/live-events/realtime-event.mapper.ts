...
export function mapRealtimeEvent(
  ...
  if (path === "/uiupdatehub") {
    ...
    if (source.type === "ui.warning")
      return message(
        "UiWarning",
        { tournamentId: source.tournamentId, ...(source.payload as object) },
        source.sequence,
      );
  ...
  if (path === "/lobbygateway") {
    if (source.type === "syncstart.connection-status")
      return message(
        ...
        source.payload as SyncStartConnectionStatusDto,
        source.sequence,
      );
    if (source.type === "syncstart.lobby-active")
      return message(
        "OnConnectionActive",
        source.payload as LobbyConnectionDto,
        source.sequence,
      );
    ...
    if (source.type === "syncstart.player-ready")
      return message(
        "OnPlayerReady",
        source.payload as LobbyPlayerReadyDto,
        source.sequence,
      );
    return sequenceOnly();
  }
