...
import {
  ...
  SongInput,
  SongStore,
} from '@tournament/catalog/song.store';
...
export class SongCommands {
  ...
  async create(input: SongInput): Promise<number> {
    const tournament = input.tournamentId
      ? await this.store.loadTournament(input.tournamentId)
      : null;
