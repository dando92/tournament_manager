...
function internalDependencies(pkg) {
    return Object.keys({ ...pkg.dependencies, ...pkg.devDependencies }).filter((name) => name.startsWith('@tournament-manager/'));
}
...
for (const directory of [join(root, 'apps', 'frontend', 'src'), join(root, 'apps', 'realtime', 'src')]) {
...
const localRealtimeReplicas = (localCompose.match(/^ {2}realtime[a-z0-9-]*:$/gm) ?? []).length;
if (localRealtimeReplicas < 2) {
    errors.push('local Compose must run at least two realtime replicas; they verify that Pub/Sub fan-out converges without client affinity');
}
