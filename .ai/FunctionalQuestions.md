...
### FQ-020 — What "advanced" means for somebody the pool never seated

- Status: Open. Narrowed by the PhaseGroup aggregate on 2026-08-23.
- Observed behavior: a pool's roster has two sources. A **seat** — a `phase_group_entrant` row — is what the pool decided: the order a generated bracket seated people in, or a placement an advancement rule produced. Playing in a match of the pool is the other, and it is derived from the matches by `PhaseGroupQueries.entrants` rather than copied into a seat row, which is what `syncDerivedEntrants` used to do after every match write. When a pool completes, `PhaseGroupAggregate.markAdvanced` marks the seats of whoever the rules moved on. Somebody who is in the pool only because a match holds them has no seat, so nothing records that they advanced, and the roster reports them as `active` with a null seed and slot. Before this they had a copied row and were marked. Nothing in the application reads that status: `GET /phase-groups/:id/entrants` has no client, and the tree counts a division's entrants rather than a pool's.
